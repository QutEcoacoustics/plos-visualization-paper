# The radarPlot function is a slight adaptation of radarchart in fmsb
# Nakazawa, M. (2015). 'fmsb' (R package) Version 0.5.2 
# Retrieved from https://cran.r-project.org/web/packages/fmsb/fmsb.pdf
# The code indicates the lines where the code has been added (++++) or
# changed (CCCC) from the original. The changes add additional control to
# the amount of white space that is added around each plot
                       ...
                       palcex = NULL, y1 = 1.2, x1 = 1.2, y2 = 1.2, x2 = 1.2, ...) # (++++)
{
  ...
  if (!is.null(vlabels)) 
    VLABELS <- vlabels
  if (is.null(vlcex)) 
    text(xx * 1.16, yy * 1.16, VLABELS)  #(CCCC)
  else text(xx * 1.16, yy * 1.16, VLABELS, cex = vlcex) #(CCCC)
  series <- length(df[[1]])
  SX <- series - 2
  if (length(pty) < SX) {
    ptys <- rep(pty, SX)
